...
class _FlashcardScreenState extends State<FlashcardScreen> {
  ...
  AppBar _buildAppBar(
    ...
    return AppBar(
      ...
      leading: IconButton(
        ...
        onPressed: () async {
          // reset provider / data first
          await provider.restartDeck(widget.category);

          // reset UI-only state locally
          setState(() {
            _showAnswer = false;
  ...
  Widget _buildDeck(Size size, List<Flashcard> flashcards, int currentIndex,
    ...
    const maxPeekCards = 3; // only show top + 2 more behind
        ...
        child: GestureDetector(
          ...
          child: Stack(
            ...
            children: flashcards
                .asMap()
                .entries
                .where((entry) => entry.key >= currentIndex && entry.key < currentIndex + maxPeekCards)
                .map((entry) {
                      ...
                      child: Stack(
                        children: [
                          ...
                          Positioned(
                            ...
                            child: IconButton(
                              icon: Icon(
                                card.safeFavorite ? Icons.star : Icons.star_border,
                                color: Colors.orange,
                              ),
                              onPressed: () => provider.toggleFavorite(card, widget.category),
                            ),
                          ),
                        ],
                      ),
  ...
  Scaffold _deckCompleteView(
      FlashcardProvider provider, String category) {
    return Scaffold(
      backgroundColor: Theme.of(context).scaffoldBackgroundColor,
  ...
  Future<void> _showEditDialog(FlashcardProvider provider, Flashcard card) async {
